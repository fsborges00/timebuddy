...
Time Buddy is a GitHub Pages-ready static web app for planning times across multiple time zones with a configurable base timezone:

- `ET` -> `America/New_York`
- `CT` -> `America/Chicago`

By default, the app starts in `ET`.

It shows:
- Base timezone current time (`ET` or `CT`)
- Multiple selected timezone times
- Time difference versus the selected base timezone (ahead/behind)

It also supports:
- **Now** mode (auto-updates every second)
- **Pick date/time** mode with selectable input timezone
- Base timezone selector (`ET` / `CT`) in the converter controls
...
- UTC offset + DST/Standard status shown on each result row
- DST transition warning in Custom mode when selected date has a DST shift in the input timezone
- Copy buttons for:
  - Full time list
...
Use the **Anchor timezone** dropdown in the main controls:

- Select `ET` for `America/New_York`
- Select `CT` for `America/Chicago`

All calculations (result anchor row, difference text, business-hours overlap timeline anchor, and subject preview anchor) follow the selected option.

## Quality Criteria (lightweight, production-focused)

To match the reliability expectations of top time-converter tools without bloating the app:

- Time conversion uses IANA timezone identifiers via Luxon (`setZone` / `fromISO` with `zone`).
- Every rendered row surfaces:
  - timezone abbreviation
  - numeric UTC offset (for unambiguous comparison)
  - DST status (`DST` or `Standard`)
- In **Custom** mode, the app warns when the selected date has a DST offset transition in the chosen input timezone.
- The app keeps a single source of truth for base timezone (`ET`/`CT`) to avoid drift between features.
...
- Base timezone defaults to `ET` unless you switch to `CT`.
